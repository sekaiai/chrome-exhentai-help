# Exhentai 辅助浏览 Chrome 扩展插件
同时支持exhentai和e-hentai

## 核心功能
- 批量下载：一键获取当前页面所有种子或图片，无需逐个点击；
- 自动加载：滚动至页面底部自动加载下一页，实现无缝浏览。

## 安装方式
### 1. Chrome 商店安装（需登录 Chrome商店）
官方链接：http://chromewebstore.google.com/detail/chrome-exhentai-help/dpnkmdlkpcgckdjolnddjgnahcagjlmd

### 2. 手动安装
1. 下载安装包：访问 https://github.com/sekaiai/chrome-exhentai-help/releases，下载 `chrome-exhentai-help.zip` 压缩包；
2. 解压压缩包，得到 `build` 文件夹（可重命名）；
3. 打开 Chrome 扩展管理页：输入 `chrome://extensions/` 或通过「菜单→更多工具→扩展程序」进入；
4. 开启右上角「开发者模式」；
5. 点击「加载已解压的扩展程序」，选择解压后的 `build` 文件夹，确认即可。

## 使用方法
1. 安装后，浏览器右上角会显示扩展图标（可通过「拼图」图标固定）；
2. 访问 Exhentai 网站，扩展自动激活；
3. 批量下载：点击扩展图标，选择对应下载选项；
4. 自动加载：滚动页面至底部，自动触发下一页加载。

## 注意事项
- 仅适配 Exhentai 网站；
- 需保持「开发者模式」开启，关闭可能导致扩展失效；
